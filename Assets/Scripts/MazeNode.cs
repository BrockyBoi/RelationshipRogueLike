...
namespace Maze
{
    ...
    public class MazeNode : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (MazeGenerator.Instance.ShouldShake)
            {
                Vector2 offset = MazeGenerator.Instance.OffsetPos;
                transform.position = _startPos + new Vector3(offset.x, 0, offset.y);
            }
            else if (MazeGenerator.Instance.ShouldRotate)
            {
                GameObject mazeNodes = ParentObjectsManager.Instance.MazeNodesParent;
                transform.RotateAround(mazeNodes.transform.position, Vector3.up, MazeGenerator.Instance.RotationRate);
            }
        }
        ...
        private void OnTriggerEnter(Collider other)
        {
            if (IsEndNode || IsStartNode)
            {
                if (IsStartNode)
                {
                    //Debug.Log("On enter start node");
                }
                else
                {
                    //Debug.Log("On enter end node");
                }
        ...
        private void OnTriggerExit(Collider other)
        {
            if (IsEndNode || IsStartNode)
            {
                if (IsStartNode)
                {
                    //Debug.Log("On exit start node");
                }
                else
                {
                    //Debug.Log("On exit end node");
                }
